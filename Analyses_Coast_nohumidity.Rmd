...
# Model cases
model.initial.cases = glmer(cases ~ (1|state) + coastal.distance + offset(log(population2019)) + scale(popdensity) + scale(poverty) + scale(log(median_income)) + scale(pct_obesity) + scale(voter_margin_2020) + scale(median_age) + factor(party) + mean_pm25 , 
                            family = poisson(link = "log"), data = coastal.only)
summary(model.initial.cases)

# Model deaths
model.initial.deaths = glmer(deaths ~ (1|state) + coastal.distance + offset(log(population2019)) + scale(popdensity) + scale(poverty) + scale(log(median_income)) + scale(pct_obesity) + scale(voter_margin_2020) + scale(median_age) + factor(party)  + mean_pm25, family = poisson(link = "log"), data = coastal.only)
